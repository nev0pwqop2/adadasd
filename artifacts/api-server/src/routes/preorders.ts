...
// POST /api/preorders/create-stripe — create Stripe session for pre-order
router.post("/create-stripe", requireAuth, async (req: Request, res: Response) => {
  ...
  // Check if user already has a paid preorder
  ...
  const chargeAmount = pricePerDay;
      ...
      line_items: [
        {
          price_data: {
            ...
            product_data: {
              ...
              description: `Pre-order a slot for the next available opening at $${chargeAmount.toFixed(2)}`,
            },
          },
          quantity: 1,
        },
...
// POST /api/preorders/create-crypto — create crypto payment for pre-order
...
// DELETE /api/preorders/:id — cancel my pre-order
router.delete("/:id", requireAuth, async (req: Request, res: Response) => {
  ...
  try {
    const rows = await db.select().from(preordersTable)
      .where(and(eq(preordersTable.id, id), eq(preordersTable.userId, req.session.userId!)))
      .limit(1);
    ...
    await db.update(preordersTable)
      .set({ status: "cancelled" })
      .where(eq(preordersTable.id, id));

    res.json({ success: true });
  } catch (err) {
    req.log.error({ err }, "Failed to cancel pre-order");
    res.status(500).json({ error: "server_error" });
  }
});
