...
async function completeBalanceDeposit(
  ...
  await db
    .update(usersTable)
    .set({
      balance: sql`${usersTable.balance} + ${depositAmount.toFixed(2)}::numeric`,
      updatedAt: new Date(),
    })
  ...
  logger.info(
    { paymentId, userId, depositAmount },
    "Payment poller: balance deposit auto-completed",
  );
}
...
async function completeSlotPayment(
  ...
  logger.info(
    { paymentId, userId, slotNumber },
    "Payment poller: slot payment auto-completed",
  );
}
  ...
  const pending = await db
    ...
    .where(
      and(
        ...
        inArray(paymentsTable.method, [
          "stripe",
          "balance-deposit-stripe",
          "preorder-stripe",
        ]),
    ...
    try {
      ...
      if (isBalanceDeposit) {
        ...
        await activateSlotShared(
          ...
          payment.slotNumber,
          payment.id,
          payment.derivationIndex ?? undefined,
        );
      } else {
        await db
          .update(paymentsTable)
        ...
        logger.info(
          ...
          "Payment poller: stripe payment marked complete",
        );
      }
    } catch (err) {
      logger.warn(
        { err, paymentId: payment.id },
        "Payment poller: stripe check failed for payment",
      );
    }
  ...
  for (const payment of pending) {
    ...
    try {
      const res = await fetch(
        `https://api.nowpayments.io/v1/payment/${payment.txHash}`,
        {
      ...
      logger.warn(
        { err, paymentId: payment.id },
        "Payment poller: crypto check failed for payment",
      );
    }
