...
async function getAvailableSlot(preferredSlot?: number): Promise<number | null> {
  const settingsRes = await db.query(
    `SELECT value FROM settings WHERE key = 'slotCount' LIMIT 1`
  );
  const slotCount =
    settingsRes.rows.length > 0 ? parseInt(settingsRes.rows[0].value) : 10;
  ...
  const activeRes = await db.query(
    `SELECT DISTINCT slot_number FROM slots WHERE is_active = true`
  );
...
async function handleWhitelist(interaction: ChatInputCommandInteraction) {
  ...
  if (userRes.rows.length === 0) {
    await interaction.editReply(
      `❌ User **${username}** not found. They must log in to the site at least once first.`
    );
    return;
  }
  ...
  console.log(
    `[WHITELIST] ${interaction.user.username} whitelisted "${username}" on slot #${slotNumber} for ${hours}h`
  );
