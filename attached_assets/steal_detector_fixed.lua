...
hookfunction(TeleportService.TeleportToPlaceInstance, function(self, placeId, instanceId, ...)
    if capturing and capturingFor then
        ...
        local brainrotName = capturingFor.name
        local money = capturingFor.money
        print("[StealDetector] captured jobId=" .. jobId .. " for brainrot=" .. brainrotName)
        local fields = {
            {name = "brainrot", value = brainrotName, inline = true},
            {name = "generation", value = money and formatMoney(money) or "N/A", inline = true},
...
local function processFrame(frame)
    ...
    print("[StealDetector] found brainrot: " .. brainrot .. " | simulating click to get job id")
...
task.spawn(function()
    task.wait(2)
    print("[StealDetector] scanning GUI for brainrots...")
    scanAll()
    print("[StealDetector] scan complete, watching for new frames")
end)
